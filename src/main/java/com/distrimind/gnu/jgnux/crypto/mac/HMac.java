...
public class HMac extends BaseMac implements Cloneable {
	public static final String USE_WITH_PKCS5_V2 = "com.distrimind.gnu.crypto.hmac.pkcs5";
	...
	public void init(Map<Object, Object> attributes) throws InvalidKeyException, IllegalStateException {
		...
		if (K.length > blockSize) {
			// (0) replace K with HASH(K) if K is larger than the hash's block
			// size.
		...
		if (K.length < blockSize) {
			// (1) append zeros to the end of K to create a B byte string (e.g.,
			// if
	...
	public boolean selfTest() {
		if (valid == null) {
			try {
				...
				byte[] k1 = new byte[] { 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B,
						0x0B, 0x0B, 0x0B };
				byte[] k3 = new byte[] { (byte) 0xAA, (byte) 0xAA, (byte) 0xAA, (byte) 0xAA, (byte) 0xAA, (byte) 0xAA,
				...
				// test #2 is not used since it causes a "Key too short"
				// exception

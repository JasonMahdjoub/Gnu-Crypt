 ...
 * <p>
 * To encrypt: <blockquote><i>C<sub>i</sub> = E<sub>k3</sub> (
 * E<sub>k2</sub><sup>-1</sup> ( E<sub>k1</sub> ( P<sub>i</sub> )))</i>
 * </blockquote>
 * <p>
 * And to decrypt: <blockquote><i>P<sub>i</sub> = E<sub>k1</sub><sup>-1</sup> (
...
public class TripleDES extends BaseCipher {
	...
	/**
	 * Convenience method which calls the method with same name and three arguments,
	 * passing <code>3</code> as the value of the first parameter.
	 *
	 * @param kb
	 *            The key bytes to adjust.
	 * @param offset
	...
	/**
	 * Adjusts, in-situ, the parity of the designated bytes, so they can be used as
	 * DES keys for a 3-DES 1-, 2- or 3-key cipher.
	 *
	 * @param keyCount
	 *            the number of independent DES keys. Can be either <code>1</code>,
	 *            <code>2</code> or <code>3</code>. Any other value will cause an
	 *            {@link IllegalArgumentException} to be raised.
	 * @param kb
	 *            the array containing the key bytes to adjust. MUST have at least
	 ...
	 *
	 * @param kb
	 *            The key bytes to test.
	 * @param offset
	 *            The starting offset into the key bytes.
	 * @return <code>true</code> if the bytes in <i>kb</i> starting at <i>offset</i>
	 *         are parity adjusted.
	...
	/**
	 * Tests if enough bytes, expected to be used as DES keys for a 3-DES 1-, 2- or
	 * 3-key cipher, located in a designated byte array, has already been parity
	 * adjusted.
	 *
	 * @param keyCount
	 *            the number of independent DES keys. Can be either <code>1</code>,
	 ...
	 *            {@link IllegalArgumentException} to be raised.
	 * @param kb
	 *            the array containing the key bytes to test. MUST have at least
	 *            <code>8 * keyCount</code> bytes starting at offset position
	 *            <code>offset</code>, otherwise an
	 *            {@link ArrayIndexOutOfBoundsException} will be raised.
	 * @param offset

...
import com.distrimind.gnu.jgnux.crypto.key.KeyAgreementFactory;
import com.distrimind.gnu.jgnux.crypto.key.OutgoingMessage;
import com.distrimind.gnu.jgnux.crypto.key.srp6.SRP6KeyAgreement;
...
import com.distrimind.gnu.jgnux.crypto.sasl.InputBuffer;
import com.distrimind.gnu.jgnux.crypto.sasl.IntegrityException;
import com.distrimind.gnu.jgnux.crypto.sasl.OutputBuffer;
import com.distrimind.gnu.jgnux.security.auth.Password;
...
public class SRPClient extends ClientMechanism implements SaslClient {
	...
	private String createO(final String aol) throws AuthenticationException {
		...
		if (integrity)
			sb.append(SRPRegistry.OPTION_INTEGRITY).append("=").append(chosenIntegrityAlgorithm).append(",");
		if (confidentiality)
			sb.append(SRPRegistry.OPTION_CONFIDENTIALITY).append("=").append(chosenConfidentialityAlgorithm)
					.append(",");
			...
			if ((!properties.containsKey(Registry.SASL_USERNAME))
				...
				if (properties.containsKey(Registry.SASL_PASSWORD)) {
					...
					if (pw instanceof char[])
						...
						throw new IllegalArgumentException(pw.getClass().getName() + "is not a valid password class");
				} else {
					final PasswordCallback pwdCB = new PasswordCallback("password: ", false);
	...
	private void setupSecurityServices(final boolean sessionReUse) throws SaslException {
		...
		if (sid != null && sid.length != 0) { // update the security context and save in map
			ClientStore.instance().cacheSession(uid, ttl, new SecurityContext(srp.getAlgorithm(), sid, K, cIV, sIV,

 ...
 * <ol>
 * <li><a href=
 * "http://planeta.terra.com.br/informatica/paulobarreto/KhazadPage.html">The
...
public final class Khazad extends BaseCipher {
	...
	static {
		...
		{
			rc[i][0] = S[j++] << 24 | (S[j++] & 0xFF) << 16 | (S[j++] & 0xFF) << 8 | (S[j++] & 0xFF);
			rc[i][1] = S[j++] << 24 | (S[j++] & 0xFF) << 16 | (S[j++] & 0xFF) << 8 | (S[j++] & 0xFF);
		}
	...
	private static void khazad(byte[] in, int i, byte[] out, int j, int[][] K) {
		...
		int k0 = K[0][0];
		int k1 = K[0][1];
		int a0 = (in[i++] << 24 | (in[i++] & 0xFF) << 16 | (in[i++] & 0xFF) << 8 | (in[i++] & 0xFF)) ^ k0;
		int a1 = (in[i++] << 24 | (in[i++] & 0xFF) << 16 | (in[i++] & 0xFF) << 8 | (in[i] & 0xFF)) ^ k1;
		int b0, b1;
		// round function
		for (int r = 1; r < R; r++) {
			k0 = K[r][0];
			k1 = K[r][1];
			b0 = T0[a0 >>> 24] ^ T1[(a0 >>> 16) & 0xFF] ^ T2[(a0 >>> 8) & 0xFF] ^ T3[a0 & 0xFF] ^ T4[a1 >>> 24]
					^ T5[(a1 >>> 16) & 0xFF] ^ T6[(a1 >>> 8) & 0xFF] ^ T7[a1 & 0xFF] ^ k0;
			b1 = T0[a1 >>> 24] ^ T1[(a1 >>> 16) & 0xFF] ^ T2[(a1 >>> 8) & 0xFF] ^ T3[a1 & 0xFF] ^ T4[a0 >>> 24]
					^ T5[(a0 >>> 16) & 0xFF] ^ T6[(a0 >>> 8) & 0xFF] ^ T7[a0 & 0xFF] ^ k1;
			a0 = b0;
			a1 = b1;
		}
	 ...
	 *
	 * @param uk
	 *            the 128-bit user-supplied key material.
	 * @param bs
	...
	public Object makeKey(byte[] uk, int bs) throws InvalidKeyException {
		...
		i = 0;
		k20 = uk[i++] << 24 | (uk[i++] & 0xFF) << 16 | (uk[i++] & 0xFF) << 8 | (uk[i++] & 0xFF);
		k21 = uk[i++] << 24 | (uk[i++] & 0xFF) << 16 | (uk[i++] & 0xFF) << 8 | (uk[i++] & 0xFF);
		k10 = uk[i++] << 24 | (uk[i++] & 0xFF) << 16 | (uk[i++] & 0xFF) << 8 | (uk[i++] & 0xFF);
		k11 = uk[i++] << 24 | (uk[i++] & 0xFF) << 16 | (uk[i++] & 0xFF) << 8 | (uk[i++] & 0xFF);
		for (r = 0, i = 0; r <= R; r++) {
			rc0 = rc[r][0];
			rc1 = rc[r][1];
			kr0 = T0[k10 >>> 24] ^ T1[(k10 >>> 16) & 0xFF] ^ T2[(k10 >>> 8) & 0xFF] ^ T3[k10 & 0xFF]
					^ T4[(k11 >>> 24) & 0xFF] ^ T5[(k11 >>> 16) & 0xFF] ^ T6[(k11 >>> 8) & 0xFF] ^ T7[k11 & 0xFF] ^ rc0
					^ k20;
			kr1 = T0[k11 >>> 24] ^ T1[(k11 >>> 16) & 0xFF] ^ T2[(k11 >>> 8) & 0xFF] ^ T3[k11 & 0xFF]
					^ T4[(k10 >>> 24) & 0xFF] ^ T5[(k10 >>> 16) & 0xFF] ^ T6[(k10 >>> 8) & 0xFF] ^ T7[k10 & 0xFF] ^ rc1
					^ k21;
			Ke[r][0] = kr0;
			Ke[r][1] = kr1;
			k20 = k10;
			...
			if (r == 0 || r == R) {
				...
				Kd[R - r][0] = T0[S[kr0 >>> 24] & 0xFF] ^ T1[S[(kr0 >>> 16) & 0xFF] & 0xFF]
						^ T2[S[(kr0 >>> 8) & 0xFF] & 0xFF] ^ T3[S[kr0 & 0xFF] & 0xFF] ^ T4[S[kr1 >>> 24] & 0xFF]
						^ T5[S[(kr1 >>> 16) & 0xFF] & 0xFF] ^ T6[S[(kr1 >>> 8) & 0xFF] & 0xFF]
						^ T7[S[kr1 & 0xFF] & 0xFF];
				Kd[R - r][1] = T0[S[kr1 >>> 24] & 0xFF] ^ T1[S[(kr1 >>> 16) & 0xFF] & 0xFF]
						^ T2[S[(kr1 >>> 8) & 0xFF] & 0xFF] ^ T3[S[kr1 & 0xFF] & 0xFF] ^ T4[S[kr0 >>> 24] & 0xFF]
						^ T5[S[(kr0 >>> 16) & 0xFF] & 0xFF] ^ T6[S[(kr0 >>> 8) & 0xFF] & 0xFF]
						^ T7[S[kr0 & 0xFF] & 0xFF];
			}
		}

...
import com.distrimind.gnu.jgnu.security.prng.RandomEvent;
import com.distrimind.gnu.jgnu.security.prng.RandomEventListener;
 ...
 * an interface for adding random events. Applications that use this code
 * <em>must</em> provide this mechanism. We use this design because an
 ...
 * <dd>This class implements {@link Serializable} in such a way that it writes a
 * 64 byte seed to the stream, and reads it back again when being deserialized.
 * This is the extent of seed file management, however, and those using this
 * class are encouraged to think deeply about when, how often, and where to
 * store the seed.</dd>
 * </dl>
 * <p>
 * <b>References:</b>
...
public class Fortuna extends BasePRNG implements Serializable, RandomEventListener {
	...
	public static class Generator extends BasePRNG implements Cloneable {
		...
		/**
		 * Resets the cipher's key. This is done after every reseed, which combines the
		 * old key and the seed, and processes that throigh the hash function.
		 */
		private void resetKey() {
			try {
				cipher.reset();
				cipher.init(Collections.singletonMap((Object) IBlockCipher.KEY_MATERIAL, (Object) key));
			}
	...
	public static final String SEED = "com.distrimind.gnu.crypto.prng.fortuna.seed";
	...
	public void fillBlock() throws LimitReachedException {
		if (pool0Count >= MIN_POOL_SIZE && System.currentTimeMillis() - lastReseed > 100) {
			...
			for (int i = 0; i < NUM_POOLS; i++)
			{
				...
				{
					refreshDigestWithRandomEvents(pools[i]);

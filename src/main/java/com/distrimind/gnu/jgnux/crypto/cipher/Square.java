 ...
 * References:
 * <ol>
 * <li><a href="http://www.esat.kuleuven.ac.be/~rijmen/square/">The block cipher
 * Square</a>.<br>
...
public final class Square extends BaseCipher {
	...
	static {
		...
		// Notes:
		// (1) The function mul() computes the product of two elements of
		// GF(2**8)
	 ...
	 * as the modulus: p = (a * b) mod ROOT. ROOT Generates a suitable Galois Field
	 * in GF(2**8).
	 * <p>
	 * For best performance call it with abs(b) &lt; abs(a).
	 *
	 * @param a
	 *            operand for multiply.
	 * @param b
	 *            operand for multiply.
	 ...
	 *
	 * @param x
	 *            the 32-bit data to rotate
	 * @param s
	 *            number of places to left-rotate by
	 ...
	 *            the 32-bit data to rotate
	 * @param s
	 *            number of places to right-rotate by
	 * @return the newly permutated value.
	 */
	...
	private static void square(byte[] in, int i, byte[] out, int j, int[][] K, int[] T, byte[] S) {
		...
		for (r = 1; r < ROUNDS; r++) { // R - 1 full rounds
			aa = T[(a >>> 24)] ^ rot32R(T[(b >>> 24)], 8) ^ rot32R(T[(c >>> 24)], 16) ^ rot32R(T[(d >>> 24)], 24)
					^ K[r][0];
			bb = T[(a >>> 16) & 0xFF] ^ rot32R(T[(b >>> 16) & 0xFF], 8) ^ rot32R(T[(c >>> 16) & 0xFF], 16)
					^ rot32R(T[(d >>> 16) & 0xFF], 24) ^ K[r][1];
			cc = T[(a >>> 8) & 0xFF] ^ rot32R(T[(b >>> 8) & 0xFF], 8) ^ rot32R(T[(c >>> 8) & 0xFF], 16)
		...
		aa = ((S[(a >>> 24)]) << 24 | (S[(b >>> 24)] & 0xFF) << 16 | (S[(c >>> 24)] & 0xFF) << 8
				| (S[(d >>> 24)] & 0xFF)) ^ K[r][0];
		bb = ((S[(a >>> 16) & 0xFF]) << 24 | (S[(b >>> 16) & 0xFF] & 0xFF) << 16 | (S[(c >>> 16) & 0xFF] & 0xFF) << 8
				| (S[(d >>> 16) & 0xFF] & 0xFF)) ^ K[r][1];
		cc = ((S[(a >>> 8) & 0xFF]) << 24 | (S[(b >>> 8) & 0xFF] & 0xFF) << 16 | (S[(c >>> 8) & 0xFF] & 0xFF) << 8
				| (S[(d >>> 8) & 0xFF] & 0xFF)) ^ K[r][2];
		dd = ((S[a & 0xFF]) << 24 | (S[b & 0xFF] & 0xFF) << 16 | (S[c & 0xFF] & 0xFF) << 8 | (S[d & 0xFF] & 0xFF))
				^ K[r][3];
		out[j++] = (byte) (aa >>> 24);
		out[j++] = (byte) (aa >>> 16);
		out[j++] = (byte) (aa >>> 8);
	 ...
	 * 		k = mul(in[i] &gt;&gt;&gt; 24, G[0][j]) &circ; mul(in[i] &gt;&gt;&gt; 16, G[1][j]) &circ; mul(in[i] &gt;&gt;&gt; 8, G[2][j])
	 * 				&circ; mul(in[i], G[3][j]);
	 * 		out[i] &circ;= k &lt;&lt; n;
	 * 	}
	 * }

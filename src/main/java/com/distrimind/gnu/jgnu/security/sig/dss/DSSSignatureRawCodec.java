...
import com.distrimind.gnu.jgnu.security.sig.ISignatureCodec;
...
public class DSSSignatureRawCodec implements ISignatureCodec {
	...
	public Object decodeSignature(byte[] k) {
		// magic
		if (k[0] != Registry.MAGIC_RAW_DSS_SIGNATURE[0] || k[1] != Registry.MAGIC_RAW_DSS_SIGNATURE[1]
				|| k[2] != Registry.MAGIC_RAW_DSS_SIGNATURE[2] || k[3] != Registry.MAGIC_RAW_DSS_SIGNATURE[3])
	 ...
	 * library.
	 * <p>
	 * The <i>Raw</i> format for a DSA signature, in this implementation, is a byte
	 ...
	 * <code>toByteArray()</code> method on the DSS parameter <code>r</code>,</li>
	 * <li>4-byte count of following bytes representing the DSS parameter
	 * <code>s</code>,</li>
	 * <li>n-bytes representation of a {@link BigInteger} obtained by invoking the
	 * <code>toByteArray()</code> method on the DSS parameter <code>s</code>.</li>
	 * </ol>
	 *
	 * @param signature
	 *            the signature to encode, consisting of the two DSS parameters
	 *            <code>r</code> and <code>s</code> as a {@link BigInteger} array.
	 * @return the <i>Raw</i> format encoding of the designated signature.
	 * @exception IllegalArgumentException
	 *                if the designated signature is not a DSS (Digital Signature
	 *                Standard) one.
	 */
	@Override

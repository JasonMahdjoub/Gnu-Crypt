...
public class RSAPSSSignatureRawCodec implements ISignatureCodec {
	...
	public Object decodeSignature(byte[] k) {
		// magic
		if (k[0] != Registry.MAGIC_RAW_RSA_PSS_SIGNATURE[0] || k[1] != Registry.MAGIC_RAW_RSA_PSS_SIGNATURE[1]
				|| k[2] != Registry.MAGIC_RAW_RSA_PSS_SIGNATURE[2] || k[3] != Registry.MAGIC_RAW_RSA_PSS_SIGNATURE[3])
	...
	/**
	 * Returns the encoded form of the designated RSA-PSS signature object according
	 * to the <i>Raw</i> format supported by this library.
	 * <p>
	 * The <i>Raw</i> format for an RSA-PSS signature, in this implementation, is a
	 ...
	 * <li>4-byte count of following bytes representing the RSA-PSS signature bytes
	 * in internet order,</li>
	 ...
	 *                if the designated signature is not an RSA-PSS one.
	 */
	@Override

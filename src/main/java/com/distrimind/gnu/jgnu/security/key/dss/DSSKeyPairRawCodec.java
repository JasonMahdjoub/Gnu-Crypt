...
public class DSSKeyPairRawCodec implements IKeyPairCodec {
	...
	public PrivateKey decodePrivateKey(byte[] k) {
		// magic
		if (k[0] != Registry.MAGIC_RAW_DSS_PRIVATE_KEY[0] || k[1] != Registry.MAGIC_RAW_DSS_PRIVATE_KEY[1]
				|| k[2] != Registry.MAGIC_RAW_DSS_PRIVATE_KEY[2] || k[3] != Registry.MAGIC_RAW_DSS_PRIVATE_KEY[3])
	...
	public PublicKey decodePublicKey(byte[] k) {
		// magic
		if (k[0] != Registry.MAGIC_RAW_DSS_PUBLIC_KEY[0] || k[1] != Registry.MAGIC_RAW_DSS_PUBLIC_KEY[1]
				|| k[2] != Registry.MAGIC_RAW_DSS_PUBLIC_KEY[2] || k[3] != Registry.MAGIC_RAW_DSS_PUBLIC_KEY[3])
			throw new IllegalArgumentException("magic");
	 ...
	 * <code>toByteArray()</code> method on the DSA parameter <code>p</code>,</li>
	 * <li>4-byte count of following bytes representing the DSA parameter
	 * <code>q</code>,</li>
	 * <li>n-bytes representation of a {@link BigInteger} obtained by invoking the
	 * <code>toByteArray()</code> method on the DSA parameter <code>q</code>,</li>
	 * <li>4-byte count of following bytes representing the DSA parameter
	 * <code>g</code>,</li>
	 * <li>n-bytes representation of a {@link BigInteger} obtained by invoking the
	 * <code>toByteArray()</code> method on the DSA parameter <code>g</code>,</li>
	 * <li>4-byte count of following bytes representing the DSA parameter
	 * <code>x</code>,</li>
	 * <li>n-bytes representation of a {@link BigInteger} obtained by invoking the
	 * <code>toByteArray()</code> method on the DSA parameter <code>x</code>,</li>
	 * </ol>
	 *
	 * @param key
	 *            the key to encode.
	 ...
	 *             if the designated key is not a DSS (Digital Signature Standard)
	 *             one.
	 */
	@Override
	 ...
	 * <p>
	 * The <i>Raw</i> format for a DSA public key, in this implementation, is a byte
	 ...
	 * <code>y</code>,</li>
	 * <li>n-bytes representation of a {@link BigInteger} obtained by invoking the
	 * <code>toByteArray()</code> method on the DSA parameter <code>y</code>,</li>
	 * </ol>
	 *
	 * @param key
	 *            the key to encode.

...
public final class Properties
{
     ...
     * block ciphers check, or not, for possible/potential weak and semi-weak
	...
	if (sm != null)
	    sm.checkPermission(
		    new PropertyPermission(REPRODUCIBLE_PRNG, "read"));
     ...
     *
     * @param value
     *            if <code>true</code> then the cipher implementations will
     *            invoke additional checks for weak and semi-weak key values
	...
	if (sm != null)
	    sm.checkPermission(
		    new PropertyPermission(CHECK_WEAK_KEYS, "write"));
	...
	if (sm != null)
	    sm.checkPermission(
		    new PropertyPermission(DO_RSA_BLINDING, "write"));
	instance().doRSABlinding = value;
	props.put(DO_RSA_BLINDING, String.valueOf(value));
    }

    /**
     * Sets the value of a designated library global configuration property, to
     * a string representation of what should be a legal value.
     *
     * @param key
     *            the case-insensitive, non-null and non-empty name of a
     *            configuration property.
     * @param value
     *            the non-null, non-empty string representation of a legal value
	...
	if (key.equals(REPRODUCIBLE_PRNG) && (value.equalsIgnoreCase(TRUE)
		|| value.equalsIgnoreCase(FALSE)))
	    setReproducible(Boolean.valueOf(value).booleanValue());
	else if (key.equals(CHECK_WEAK_KEYS) && (value.equalsIgnoreCase(TRUE)
		|| value.equalsIgnoreCase(FALSE)))
	    setCheckForWeakKeys(Boolean.valueOf(value).booleanValue());
	else if (key.equals(DO_RSA_BLINDING) && (value.equalsIgnoreCase(TRUE)
		|| value.equalsIgnoreCase(FALSE)))
     ...
     *
     * @param value
     *            if <code>true</code> then the default PRNG bit stream output
	...
	if (sm != null)
	    sm.checkPermission(
		    new PropertyPermission(REPRODUCIBLE_PRNG, "write"));
	...
	if (propFile != null)
	{
		...
		for (Iterator<Entry<Object, Object>> it = temp.entrySet()
			.iterator(); it.hasNext();)
		{
	...
	reproducible = Boolean.valueOf(props.get(REPRODUCIBLE_PRNG))
		.booleanValue();
	checkForWeakKeys = Boolean.valueOf(props.get(CHECK_WEAK_KEYS))
		.booleanValue();
	doRSABlinding = Boolean.valueOf(props.get(DO_RSA_BLINDING))
		.booleanValue();

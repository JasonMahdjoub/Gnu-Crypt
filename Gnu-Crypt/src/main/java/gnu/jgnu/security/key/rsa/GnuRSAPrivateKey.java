...
public class GnuRSAPrivateKey extends GnuRSAKey implements PrivateKey, RSAPrivateCrtKey
{
     ...
     * <code>encodePrivateKey()</code> method of an RSA keypair codec object (an
	...
	if (k[0] == Registry.MAGIC_RAW_RSA_PRIVATE_KEY[0])
	    try
	    {
		return (GnuRSAPrivateKey) new RSAKeyPairRawCodec()
			.decodePrivateKey(k);
	    }
	...
	// try PKCS#8 codec
	return (GnuRSAPrivateKey) new RSAKeyPairPKCS8Codec()
		.decodePrivateKey(k);
    }
     ...
     *            externalizing this key.
     * @param n
     *            the public modulus, which is also the product of
     ...
     * @param qInv
     *            the Chinese Remainder Theorem coefiicient. A positive integer
	...
	super(preferredFormat == Registry.ASN1_ENCODING_ID
		? Registry.PKCS8_ENCODING_ID : preferredFormat, n, e);
	this.d = d;
	this.p = p;
	this.q = q;
	...
	if (str == null)
	{
	    ...
	    str = new StringBuilder(this.getClass().getName()).append("(")
		    ...
		    .append("d=0x").append("**...*").append(ls).append("p=0x")
		    .append("**...*").append(ls).append("q=0x").append("**...*")
		    .append(ls).append("dP=0x").append("**...*").append(ls)
		    .append("dQ=0x").append("**...*").append(ls)
		    .append("qInv=0x").append("**...*").append(ls).append(")")
		    .toString();
	}
	return str;
    }

...
public class DSSPrivateKey extends DSSKey implements PrivateKey, DSAPrivateKey
{
     ...
     * <code>encodePrivateKey()</code> method of a DSS keypair codec object (an
	...
	if (k[0] == Registry.MAGIC_RAW_DSS_PRIVATE_KEY[0])
	    try
	    {
		return (DSSPrivateKey) new DSSKeyPairRawCodec()
			.decodePrivateKey(k);
	    }
	...
	super(preferredFormat == Registry.ASN1_ENCODING_ID
		? Registry.PKCS8_ENCODING_ID : preferredFormat, p, q, g);
	this.x = x;
    }

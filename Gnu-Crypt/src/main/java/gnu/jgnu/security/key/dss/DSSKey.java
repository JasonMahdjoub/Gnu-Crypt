...
public abstract class DSSKey implements Key, DSAKey
{
     ...
     * {@link DSAKey} and has the same DSS (Digital Signature Standard)
     * parameter values as this one.
     * <p>
     * Always returns <code>false</code> if the MPIs of this key are
     * <i>inherited</i>. This may be the case when the key is re-constructed
     * from an X.509 certificate with absent or NULL AlgorithmIdentifier's
     * parameters field.
     *
     * @param obj
	...
	if (str == null)
	{
	    ...
	    if (hasInheritedParameters())
		sb.append("p=inherited,").append(ls).append("q=inherited,")
			.append(ls).append("g=inherited");
	    else
		sb.append("p=0x").append(p.toString(16)).append(",").append(ls)
			.append("q=0x").append(q.toString(16)).append(",")

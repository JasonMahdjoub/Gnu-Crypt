 ...
 * <ol>
 * <li><a href=
 * "http://www.cosic.esat.kuleuven.ac.be/nessie/workshop/submissions/rsa-pss.zip">
...
public class RSA {
	 ...
	 *             if <code>K</code> is not an RSA one.
	 * @throws IllegalArgumentException
	 *             if <code>c</code>, the ciphertext representative is not between
	 *             <code>0</code> and <code>n - 1</code> (<code>n</code> being the
	 ...
	 *         <code>n - 1</code> (<code>n</code> being the public shared modulus)--
	 *         that will eventually be padded with an appropriate framing/padding
	 *         scheme.
	 * @throws ClassCastException
	 *             if <code>K</code> is not an RSA one.
	 * @throws IllegalArgumentException
	 *             if <code>m</code>, the message representative is not between
	 *             <code>0</code> and <code>n - 1</code> (<code>n</code> being the
	 *             public shared modulus).
	 */
	...
	/**
	 * Converts a <i>multi-precision integer</i> (MPI) <code>s</code> into an octet
	 * sequence of length <code>k</code>.
	 *
	 * @param s
	 *            the multi-precision integer to convert.
	 * @param k
	 *            the length of the output.
	 * @return the result of the transform.
	 * @exception IllegalArgumentException
	 *                if the length in octets of meaningful bytes of <code>s</code>
	...
	private static final BigInteger RSADP(final RSAPrivateKey K, BigInteger c) {
		...
		if (!(K instanceof RSAPrivateCrtKey)) {
			...
			// decryption, where r is random, e is the RSA encryption exponent,
			// and
			// g is the ciphertext to be decrypted. x is then decrypted as
			// normal,
			// followed by division by r, i.e. (x^e) / r mod n. Since r is
			// random,
			// x is random and timing the decryption should not reveal
			// information
			// about the key. Note that r should be a new random number for
			// every
			// decryption.
	 ...
	 * representative</i> for a designated <i>message representative</i> signed by
	 * the holder of the designated RSA private key.
	 *
	 * @param K
	 *            the RSA private key.
	 * @param m
	 *            the <i>message representative</i>: an integer between
	 *            <code>0</code> and <code>n - 1</code>, where <code>n</code> is the
	 *            RSA <i>modulus</i>.
	 ...
	 *             if <code>K</code> is not an RSA one.
	 * @throws IllegalArgumentException
	 *             if <code>m</code> (the <i>message representative</i>) is out of
	 *             range.
	 */
	 ...
	 * generated by an RSA private key, for a message intended for the holder of the
	 * designated RSA public key.
	 *
	 * @param K
	 *            the RSA public key.
	 * @param s
	 *            the <i>signature representative</i>, an integer between
	 *            <code>0</code> and <code>n - 1</code>, where <code>n</code> is the
	 *            RSA <i>modulus</i>.
	 ...
	 *             if <code>K</code> is not an RSA one.
	 * @throws IllegalArgumentException
	 *             if <code>s</code> (the <i>signature representative</i>) is out of
	 *             range.
	 */

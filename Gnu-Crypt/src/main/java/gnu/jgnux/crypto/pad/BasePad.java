...
public abstract class BasePad implements IPad
{
     ...
     * {@link IPad} class documentation. Other algorithm-specific attributes
     * MUST be documented in the implementation class of that padding algorithm.
     * <p>
     ...
     * an {@link UnsupportedOperationException}. Concrete padding algorithms
     * MUST override this method if they wish to offer an initialisation method
     * that allows for other than the padding block size parameter to be
     * specified.
     *
     * @param attributes
     ...
     *         what it pads for all tested block sizes. Returns
     *         <code>false</code> if the test fails for any block size.
     */
    @Override
     ...
     * @param buffer
     *            a work buffer. It is exposed as an argument for this method to
     *            reduce un-necessary object allocations.
	...
	for (int i = 0; i < limit - offset - blockSize; i++)
	{
	    ...
	    try
	    {
		if (padBytes.length != unpad(buffer, offset,
			i + padBytes.length))
		{
		    return false;
		}

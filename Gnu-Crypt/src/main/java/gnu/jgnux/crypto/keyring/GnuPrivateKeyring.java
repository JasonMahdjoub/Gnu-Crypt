...
public class GnuPrivateKeyring extends BaseKeyring implements IPrivateKeyring
{
    private static final Logger log = Logger
	    .getLogger(GnuPrivateKeyring.class.getName());
	...
	if (containsAlias(alias))
	{
		...
		for (Iterator<Entry> it = e1.getEntries().iterator(); it
			.hasNext();)
		{
		    Entry e = it.next();
		...
		if (e2 != null)
		{
		    ...
		    {
			log.throwing(this.getClass().getName(), "getPrivateKey",
				e);
			throw new UnrecoverableKeyException(
				"decryption failed");
		    }
		    for (Iterator<Entry> it = e2.get(alias).iterator(); it
			    .hasNext();)
		    {
			Entry e = it.next();
	...
	if (!containsPrivateKey(alias))
	{
	    ...
	    enc = new PasswordEncryptedEntry(cipher, mode, keylen,
		    new Properties());
	    enc.add(pke);
	    try
	    {
	    ...
	    auth = new PasswordAuthenticatedEntry(mac, maclen,
		    new Properties());
	    auth.add(enc);
	    try
	    {

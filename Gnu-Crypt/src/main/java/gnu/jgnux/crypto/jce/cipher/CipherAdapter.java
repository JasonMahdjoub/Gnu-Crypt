...
import gnu.jgnux.crypto.jce.spec.BlockCipherParameterSpec;
import gnu.jgnux.crypto.mode.IMode;
import gnu.jgnux.crypto.mode.ModeFactory;
import gnu.jgnux.crypto.pad.IPad;
import gnu.jgnux.crypto.pad.PadFactory;
 ...
 * {@link Cipher} class, which provides the functionality of symmetric-key block
 * ciphers, such as the AES.
 * <p>
 * This base class defines all of the abstract methods in {@link CipherSpi}, but
 * does not define the (non-abstract) key wrapping functions that extended the
 * base cipher SPI, and these methods thus immediately throw an
...
class CipherAdapter extends CipherSpi
{
     ...
     * argument should be the appropriate one listed in {@link Registry}. The
     * basic cipher instance is created, along with an instance of the
     * {@link gnu.jgnux.crypto.mode.ECB} mode and no padding.
     *
     * @param cipherName
     *            The cipher to instantiate.
     * @param blockLen
	...
	int l = inputLen + partLen;

	return (l / blockSize) * blockSize + blockSize;
	/*
	 * if (l%blockSize>0) res+=blockSize;
	 */
	...
	if (params == null)
	{
	    ...
	    // these cant be derived from the given key then it must be
	    // generated
	    // randomly if in ENCRYPT or WRAP mode. Parameters that have
	    // defaults
	...
	else if (params instanceof IvParameterSpec)
	{
	    ...
	    if (((IvParameterSpec) params).getIV().length != cipher
		    .defaultBlockSize())
	    {
		throw new InvalidAlgorithmParameterException();
	    }
    ...
    /**
     * Executes initialization logic after all parameters have been handled by
     * the engineInit()s.
     *
     * @param opmode
     *            the desired mode of operation for this instance.
     * @param key
     *            the key material to use for initialization.
     * @param random
     *            a source of randmoness to use if/when needed.
     * @throws InvalidKeyException
     *             if <code>key</code> is invalid or the cipher needs extra
     *             parameters which can not be derived from <code>key</code>;
     *             e.g. an IV.
     */
	...
	if (keyLen == 0)
	{
	    ...
	    // set keyLen to kbLength. either the cipher implementation will
	    // throw
	    // an InvalidKeyException, or it is implemented in a way which can
	    // deal
		...
		&& modeName.substring(0, 3).equalsIgnoreCase("CFB"))
	{
		...
		try
		{
		    ...
		    attributes.put(IMode.MODE_BLOCK_SIZE,
			    Integer.valueOf(bs / 8));
		}
	...
	if (pad != null
		&& ((Integer) attributes.get(IMode.STATE))
			.intValue() == IMode.DECRYPTION

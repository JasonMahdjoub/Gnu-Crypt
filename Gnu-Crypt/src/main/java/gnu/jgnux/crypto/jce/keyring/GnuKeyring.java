...
package gnu.jgnux.crypto.jce.keyring;
...
import gnu.jgnux.crypto.keyring.Entry;
import gnu.jgnux.crypto.keyring.GnuPrivateKeyring;
import gnu.jgnux.crypto.keyring.GnuPublicKeyring;
import gnu.jgnux.crypto.keyring.IKeyring;
import gnu.jgnux.crypto.keyring.IPrivateKeyring;
import gnu.jgnux.crypto.keyring.IPublicKeyring;
import gnu.jgnux.crypto.keyring.MalformedKeyringException;
import gnu.jgnux.crypto.keyring.PrimitiveEntry;
import gnu.vm.jgnu.security.Key;
import gnu.vm.jgnu.security.KeyStoreException;
import gnu.vm.jgnu.security.KeyStoreSpi;
...
public class GnuKeyring extends KeyStoreSpi
{
	...
	else
	{
	    ...
	    for (Enumeration<Object> e = privateKR.aliases(); e
		    .hasMoreElements();)
	    {
	    ...
	    for (Enumeration<Object> e = publicKR.aliases(); e
		    .hasMoreElements();)
	    {
		String alias = (String) e.nextElement();
		if (alias != null)
	...
	ensureLoaded();
	String result = null;
	for (Enumeration<Object> aliases = publicKR.aliases(); aliases
		.hasMoreElements();)
	{
    ...
    {
	int result = 0;
	for (Enumeration<String> e = engineAliases(); e
		.hasMoreElements(); result++)
	    e.nextElement();
    ...
    /**
     * Ensure that the underlying keyring pair is loaded. Throw an exception if
     * it isn't; otherwise returns silently.
     *
	...
	Date result = null;
	if (keyring != null)
	    for (Iterator<Entry> it = keyring.get(alias).iterator(); it
		    .hasNext();)
	    {
		Entry o = it.next();

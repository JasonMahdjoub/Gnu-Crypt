...
import gnu.jgnu.security.key.dss.DSSKeyPairPKCS8Codec;
import gnu.jgnu.security.key.dss.DSSKeyPairRawCodec;
import gnu.jgnu.security.key.dss.DSSKeyPairX509Codec;
import gnu.jgnu.security.key.rsa.RSAKeyPairPKCS8Codec;
import gnu.jgnu.security.key.rsa.RSAKeyPairRawCodec;
import gnu.jgnu.security.key.rsa.RSAKeyPairX509Codec;
import gnu.jgnux.crypto.key.dh.DHKeyPairPKCS8Codec;
import gnu.jgnux.crypto.key.dh.DHKeyPairRawCodec;
import gnu.jgnux.crypto.key.dh.DHKeyPairX509Codec;
import gnu.jgnux.crypto.key.srp6.SRPKeyPairRawCodec;
...
public class IncomingMessage {
	 ...
	 *            the encoded form, excluding the header bytes.
	 * @param offset
	 *            offset where to start using raw bytes from.
	 * @param len
	...
	private IKeyPairCodec getKeyPairCodec(int keyTypeAndFormatID) throws KeyAgreementException {
		...
		case Registry.RAW_ENCODING_ID:
			...
			default:
				throw new KeyAgreementException("Unknown key-type for Raw format: " + keyType);
			}
		case Registry.X509_ENCODING_ID:
			...
			default:
				throw new KeyAgreementException("Unknown key-type for X.509 format: " + keyType);
			}
		case Registry.PKCS8_ENCODING_ID:
			...
			default:
				throw new KeyAgreementException("Unknown key-type for PKCS#8 format: " + keyType);
			}
		default:
			throw new KeyAgreementException("Unknown format identifier: " + formatID);
		}
	}
	 ...
	 * @throws KeyAgreementException
	 *             if an encoding exception occurs during the operation.
	 */
	 ...
	 * <p>
	 * See {@link OutgoingMessage#writePrivateKey(gnu.vm.jgnu.security.PrivateKey)}
	 * for more details.
	 *
	 * @throws KeyAgreementException
	 *             if an encoding size constraint is violated or a mismatch was
	 *             detected in the encoding.
	 */
	 ...
	 * <p>
	 * See {@link OutgoingMessage#writePublicKey(gnu.vm.jgnu.security.PublicKey)}
	 * for more details on the internal format.
	 *
	 * @throws KeyAgreementException
	 *             if an encoding size constraint is violated or a mismatch was
	 *             detected in the encoding.

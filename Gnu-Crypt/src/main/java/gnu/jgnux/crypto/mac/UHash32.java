...
public class UHash32 extends BaseMac
{
	...
	public byte[] digest()
	{
		...
		if (count % 32 != 0)
		{
		    ...
		    System.arraycopy(ALL_ZEROES, 0, buffer, count,
			    limit - count);
		    count += limit - count;
		}
		byte[] y = nh32(count);
	...
	L2Hash32(byte[] K)
	{
	    ...
	    k64 = new BigInteger(1,
		    new byte[] { (byte) (K[i++] & 0x01), (byte) (K[i++] & 0xFF),
			    (byte) (K[i++] & 0xFF), (byte) (K[i++] & 0xFF),
			    (byte) (K[i++] & 0x01), (byte) (K[i++] & 0xFF),
			    (byte) (K[i++] & 0xFF), (byte) (K[i++] & 0xFF) });
	    k128 = new BigInteger(1,
		    new byte[] { (byte) (K[i++] & 0x01), (byte) (K[i++] & 0xFF),
			    (byte) (K[i++] & 0xFF), (byte) (K[i++] & 0xFF),
			    (byte) (K[i++] & 0x01), (byte) (K[i++] & 0xFF),
			    (byte) (K[i++] & 0xFF), (byte) (K[i++] & 0xFF),
	...
	switch (n)
	{
		...
		throw new IllegalArgumentException(
			"Undefined prime(" + String.valueOf(n) + ")");
	}
    }
	...
	if (K.length != UMac32.KEY_LEN)
	    throw new InvalidKeyException(
		    "Invalid Key length: " + String.valueOf(K.length));

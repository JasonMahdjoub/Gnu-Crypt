...
public class Fortuna extends BasePRNG implements Serializable, RandomEventListener
{
	...
	public void addRandomBytes(byte[] seed, int offset, int length)
	{
	    ...
	    System.arraycopy(newkey, 0, key, 0,
		    Math.min(key.length, newkey.length));
	    resetKey();
	    incrementCounter();
	    seeded = true;
	}
	...
	private void resetKey()
	{
	    ...
	    {
		cipher.reset();
		cipher.init(Collections.singletonMap(
			(Object) IBlockCipher.KEY_MATERIAL, (Object) key));
	    }
	...
	if (event.getPoolNumber() < 0 || event.getPoolNumber() >= pools.length)
	    throw new IllegalArgumentException(
		    "pool number out of range: " + event.getPoolNumber());

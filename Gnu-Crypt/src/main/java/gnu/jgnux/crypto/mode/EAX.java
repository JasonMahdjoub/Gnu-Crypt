...
public class EAX implements IAuthenticatedMode
{
	...
	ctr = ModeFactory.getInstance(Registry.CTR_MODE, cipher,
		cipherBlockSize);
	...
	nonceOmac.reset();
	nonceOmac.init(Collections.singletonMap((Object) MAC_KEY_MATERIAL,
		(Object) key));
	...
	t_n[t_n.length - 1] = 1;
	headerOmac.reset();
	headerOmac.init(Collections.singletonMap((Object) MAC_KEY_MATERIAL,
		(Object) key));
	headerOmac.update(t_n, 0, t_n.length);
	t_n[t_n.length - 1] = 2;
	msgOmac.reset();
	msgOmac.init(Collections.singletonMap((Object) MAC_KEY_MATERIAL,
		(Object) key));

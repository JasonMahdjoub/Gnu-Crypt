...
public final class Rijndael extends BaseCipher
{
    ...
    private static final byte[] KAT_CT = Util
	    .toBytesFromString("E44429474D6FC3084EB2A6B8B46AF754");
	...
	int t0 = (in[i++] << 24 | (in[i++] & 0xFF) << 16 | (in[i++] & 0xFF) << 8
		| (in[i++] & 0xFF)) ^ Kdr[0];
	int t1 = (in[i++] << 24 | (in[i++] & 0xFF) << 16 | (in[i++] & 0xFF) << 8
		| (in[i++] & 0xFF)) ^ Kdr[1];
	int t2 = (in[i++] << 24 | (in[i++] & 0xFF) << 16 | (in[i++] & 0xFF) << 8
		| (in[i++] & 0xFF)) ^ Kdr[2];
	int t3 = (in[i++] << 24 | (in[i++] & 0xFF) << 16 | (in[i++] & 0xFF) << 8
		| (in[i++] & 0xFF)) ^ Kdr[3];
	...
	for (int r = 1; r < ROUNDS; r++) // apply round transforms
	{
	    ...
	    a0 = (T5[(t0 >>> 24)] ^ T6[(t3 >>> 16) & 0xFF]
		    ^ T7[(t2 >>> 8) & 0xFF] ^ T8[t1 & 0xFF]) ^ Kdr[0];
	    a1 = (T5[(t1 >>> 24)] ^ T6[(t0 >>> 16) & 0xFF]
		    ^ T7[(t3 >>> 8) & 0xFF] ^ T8[t2 & 0xFF]) ^ Kdr[1];
	    a2 = (T5[(t2 >>> 24)] ^ T6[(t1 >>> 16) & 0xFF]
		    ^ T7[(t0 >>> 8) & 0xFF] ^ T8[t3 & 0xFF]) ^ Kdr[2];
	    a3 = (T5[(t3 >>> 24)] ^ T6[(t2 >>> 16) & 0xFF]
		    ^ T7[(t1 >>> 8) & 0xFF] ^ T8[t0 & 0xFF]) ^ Kdr[3];
	    t0 = a0;
	    t1 = a1;
	    t2 = a2;
	    t3 = a3;
	...
	{
	    Ker = Ke[r];
	    a0 = (T1[(t0 >>> 24)] ^ T2[(t1 >>> 16) & 0xFF]
		    ^ T3[(t2 >>> 8) & 0xFF] ^ T4[t3 & 0xFF]) ^ Ker[0];
	    a1 = (T1[(t1 >>> 24)] ^ T2[(t2 >>> 16) & 0xFF]
		    ^ T3[(t3 >>> 8) & 0xFF] ^ T4[t0 & 0xFF]) ^ Ker[1];
	    a2 = (T1[(t2 >>> 24)] ^ T2[(t3 >>> 16) & 0xFF]
		    ^ T3[(t0 >>> 8) & 0xFF] ^ T4[t1 & 0xFF]) ^ Ker[2];
	    a3 = (T1[(t3 >>> 24)] ^ T2[(t0 >>> 16) & 0xFF]
		    ^ T3[(t1 >>> 8) & 0xFF] ^ T4[t2 & 0xFF]) ^ Ker[3];
	    t0 = a0;
	    t1 = a1;
	    t2 = a2;
	    t3 = a3;
	...
	for (i = 0; i < BC; i++) // last round is special
	{
	    ...
	    out[outOffset++] = (byte) (Si[(t[(i + s1) % BC] >>> 16) & 0xFF]
		    ^ (tt >>> 16));
	    out[outOffset++] = (byte) (Si[(t[(i + s2) % BC] >>> 8) & 0xFF]
		    ^ (tt >>> 8));
	...
	for (i = 0; i < BC; i++) // last round is special
	{
	    ...
	    out[outOffset++] = (byte) (S[(t[(i + s1) % BC] >>> 16) & 0xFF]
		    ^ (tt >>> 16));
	    out[outOffset++] = (byte) (S[(t[(i + s2) % BC] >>> 8) & 0xFF]
		    ^ (tt >>> 8));

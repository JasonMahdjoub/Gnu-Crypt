...
public final class Square extends BaseCipher
{
    ...
    private static final byte[] KAT_KEY = Util
	    .toBytesFromString("00000000000000000000020000000000");

    private static final byte[] KAT_CT = Util
	    .toBytesFromString("A9DF031B4E25E89F527EFFF89CB0BEBA");
	...
	for (i = 0; i < 256; i++)
	{
	    ...
	    j = Sd[i] & 0xFF;
	    Td[i] = (Sd[i & 3] == 0) ? 0
		    : mul(j, 14) << 24 | mul(j, 9) << 16 | mul(j, 13) << 8
			    | mul(j, 11);
	}
    }
    ...
    {
	int a = ((in[i++]) << 24 | (in[i++] & 0xFF) << 16
		| (in[i++] & 0xFF) << 8 | (in[i++] & 0xFF)) ^ K[0][0];
	int b = ((in[i++]) << 24 | (in[i++] & 0xFF) << 16
		| (in[i++] & 0xFF) << 8 | (in[i++] & 0xFF)) ^ K[0][1];
	int c = ((in[i++]) << 24 | (in[i++] & 0xFF) << 16
		| (in[i++] & 0xFF) << 8 | (in[i++] & 0xFF)) ^ K[0][2];
	int d = ((in[i++]) << 24 | (in[i++] & 0xFF) << 16
		| (in[i++] & 0xFF) << 8 | (in[i] & 0xFF)) ^ K[0][3];
	int r, aa, bb, cc, dd;
	for (r = 1; r < ROUNDS; r++)
	{ // R - 1 full rounds
	    aa = T[(a >>> 24)] ^ rot32R(T[(b >>> 24)], 8)
		    ^ rot32R(T[(c >>> 24)], 16) ^ rot32R(T[(d >>> 24)], 24)
		    ^ K[r][0];
	...
	dd = ((S[a & 0xFF]) << 24 | (S[b & 0xFF] & 0xFF) << 16
		| (S[c & 0xFF] & 0xFF) << 8 | (S[d & 0xFF] & 0xFF)) ^ K[r][3];
	out[j++] = (byte) (aa >>> 24);
	out[j++] = (byte) (aa >>> 16);
	out[j++] = (byte) (aa >>> 8);
	out[j++] = (byte) aa;

...
public final class SRP {
	...
	private static final HashMap<String, SRP> algorithms = new HashMap<>();
	...
	public static synchronized SRP instance(String mdName) {
		...
		SRP result = algorithms.get(mdName);
		if (result == null) {
	...
	public byte[] computeX(final byte[] s, final String user, final String password)
	 ...
	 *
	 * @param src
	 *            a String whose bytes (using US-ASCII encoding) are to be digested.
	 * @return the bytes constituting the result of digesting the designated input
	 *         with a new instance of the SRP message digest algorithm.
	 * @throws UnsupportedEncodingException
	 *             if US-ASCII charset is not found.
	 */
	...
	public byte[] generateM1(final BigInteger N, final BigInteger g, final String U, final byte[] s, final BigInteger A,
			final BigInteger B, final byte[] K, final String I, final String L, final byte[] cn, final byte[] cCB)
			throws UnsupportedEncodingException {
		final IMessageDigest hash = (IMessageDigest) mda.clone();
		byte[] b;
	...
	public byte[] generateM2(final BigInteger A, final byte[] M1, final byte[] K, final String U, final String I,
			final String o, final byte[] sid, final int ttl, final byte[] cIV, final byte[] sIV, final byte[] sCB)
			throws UnsupportedEncodingException {
		final IMessageDigest hash = (IMessageDigest) mda.clone();
		byte[] b;
		b = Util.trim(A);
	...
	/**
	 * Returns a new instance of the SRP message digest algorithm --which is SHA-160
	 * by default, but could be anything else provided the proper conditions as
	 * specified in the SRP specifications.
	 *
	 * @return a new instance of the underlying SRP message digest algorithm.
	 * @throws RuntimeException
	 *             if the implementation of the message digest algorithm does not
	 *             support cloning.
	 */
	...
	/**
	 * Convenience method to XOR N bytes from two arrays; N being the output size of
	 * the SRP message digest algorithm.
	 *
	 * @param a

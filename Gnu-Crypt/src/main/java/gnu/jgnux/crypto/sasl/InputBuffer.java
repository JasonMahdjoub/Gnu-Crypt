 ...
 * <ol>
 * <li><a href=
 * "http://www.ietf.org/internet-drafts/draft-burdis-cat-srp-sasl-09.txt">
...
public class InputBuffer
{
     ...
     *
     * @param raw
     *            the encoded form, excluding the header bytes, of a SASL
     *            buffer.
	...
	if (result > Registry.SASL_TWO_BYTE_MAX_LIMIT)
	    throw new SaslEncodingException(
		    "SASL MPI/Text size limit exceeded");
	return result;
    }
     ...
     *
     * @param frame
     *            the encoded form, including the header bytes, of a SASL
     *            buffer.
	...
	if (in.available() < 2)
	    throw new SaslEncodingException(
		    "Not enough bytes for an MPI in buffer");
	byte[] elementLengthBytes = new byte[2];
	in.read(elementLengthBytes);
	final int elementLength = twoBytesToLength(elementLengthBytes);
	if (in.available() < elementLength)
	...
	if (in.available() < elementLength)
	    throw new SaslEncodingException(
		    "Illegal SASL octet-sequence encoding");
	byte[] result = new byte[elementLength];
	in.read(result);
	return result;
    }
     ...
     *             if an encoding exception occurs during the operation.
     * @throws SaslEncodingException
     *             if the UTF-8 character encoding is not supported on this
     *             platform.
     * @throws IOException
     *             if any other I/O exception occurs during the operation.
     */
	...
	if (in.available() < 2)
	    throw new SaslEncodingException(
		    "Not enough bytes for a text in buffer");
	byte[] elementLengthBytes = new byte[2];
	in.read(elementLengthBytes);
	final int elementLength = twoBytesToLength(elementLengthBytes);
	if (in.available() < elementLength)

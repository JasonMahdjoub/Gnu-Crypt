...
public abstract class ClientMechanism implements SaslClient
{
     ...
     * @throws IllegalMechanismStateException
     *             if the instance is already initialised.
     * @throws SaslException
     *             if an exception occurs during the process.
     */
    public void init(final Map<String, Object> attributes) throws SaslException
    {
	if (state != -1)
	...
	if (attributes != null)
	{
	    authorizationID = (String) attributes
		    .get(Registry.SASL_AUTHORISATION_ID);
	    protocol = (String) attributes.get(Registry.SASL_PROTOCOL);
	    serverName = (String) attributes.get(Registry.SASL_SERVER_NAME);
	    handler = (CallbackHandler) attributes
		    .get(Registry.SASL_CALLBACK_HANDLER);
	    channelBinding = (byte[]) attributes
		    .get(Registry.SASL_CHANNEL_BINDING);

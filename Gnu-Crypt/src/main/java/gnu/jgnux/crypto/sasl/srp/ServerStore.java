...
public class ServerStore
{
    ...
    private static final HashMap<String, SecurityContext> sid2ssc = new HashMap<>();

    /** The map of sid --> Session timing record. */
    private static final HashMap<String, StoreEntry> sid2ttl = new HashMap<>();
	...
	return new StringBuilder("SID-")
		.append("0000000000".substring(0, 10 - sid.length()))
		.append(sid).toString().getBytes();
    }
	...
	if (sid != null && sid.length != 0)
	{
	    ...
	    {
		final String key = new String(sid);
		final StoreEntry ctx = sid2ttl.get(key);
		if (ctx != null)
		{
		    result = ctx.isAlive();
     ...
     * return the underlying mechanism server's evidence that shall be returned
     * to the client in a session re-use exchange.
     *
     * @param sid

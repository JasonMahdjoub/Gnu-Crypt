...
import gnu.jgnu.security.hash.MD5;
...
import gnu.jgnux.crypto.sasl.ClientMechanism;
...
import gnu.jgnux.security.auth.Password;
...
public class SRPClient extends ClientMechanism implements SaslClient
{
    ...
    private IKeyAgreementParty clientHandler = KeyAgreementFactory
	    .getPartyAInstance(Registry.SRP_SASL_KA);
	...
	while (st.hasMoreTokens())
	{
		...
		for (i = 0; i < SRPRegistry.INTEGRITY_ALGORITHMS.length; i++)
		    ...
		    .startsWith(SRPRegistry.OPTION_CONFIDENTIALITY + "="))
	    {
		option = option.substring(option.indexOf('=') + 1);
		for (i = 0; i < SRPRegistry.CONFIDENTIALITY_ALGORITHMS.length; i++)
		    if (SRPRegistry.CONFIDENTIALITY_ALGORITHMS[i]
			    .equals(option))
		    {
		...
		mandatory = option.substring(option.indexOf('=') + 1);
		    ...
		    .startsWith(SRPRegistry.OPTION_MAX_BUFFER_SIZE + "="))
	    {
		final String maxBufferSize = option
	...
	else
	{
	    ...
	    final IBlockCipher cipher = CipherFactory
		    .getInstance(chosenConfidentialityAlgorithm);
	    if (cipher == null)
	...
	sb.append(SRPRegistry.OPTION_SRP_DIGEST).append("=").append(mdName)
		.append(",");
	if (replayDetection)
	...
	final String result = sb.append(SRPRegistry.OPTION_MAX_BUFFER_SIZE)
		.append("=").append(Registry.SASL_BUFFER_MAX_LIMIT).toString();
	return result;
    }
	...
	try
	{
		...
		System.arraycopy(incoming, offset + payloadLength, received_mac,
			0, macBytesCount);
		...
		if (replayDetection)
		{
		    ...
		    inMac.update(new byte[] { (byte) (inCounter >>> 24),
			    (byte) (inCounter >>> 16), (byte) (inCounter >>> 8),
			    (byte) inCounter });
		}
		...
		else
		{
		    ...
		    System.arraycopy(incoming, offset, result, 0,
			    result.length);
		}
	    }
	...
	{
	    final SecurityContext ctx = ClientStore.instance()
		    .restoreSession(uid);
	...
	try
	{
		...
		throw new SaslException("sendPublicKey(): Invalid scalar ("
			+ ack + ") in server's request");
	}
	catch (IOException x)
	{
	    if (x instanceof SaslException)
	...
	if (ack == 0x00)
	{ // new session ---------------------------------------
	    o = createO(L.toLowerCase()); // do this first to initialise the SRP
					  // hash
	    final byte[] pBytes; // use ASCII encoding to inter-operate w/
				 // non-java
	...
	{ // update the security context and save in map
	    ClientStore.instance().cacheSession(uid, ttl,
		    new SecurityContext(srp.getAlgorithm(), sid, K, cIV, sIV,
			    replayDetection, inCounter, outCounter, inMac,
			    outMac, inCipher, outCipher));
	}
    }
}

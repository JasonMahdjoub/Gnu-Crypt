...
public class SRPServer extends ServerMechanism implements SaslServer
{
    ...
    private IKeyAgreementParty serverHandler = KeyAgreementFactory
	    .getPartyBInstance(Registry.SRP_SASL_KA);
	...
	if (integrity)
	{
		...
		sb.append(SRPRegistry.OPTION_INTEGRITY).append("=")
			.append(SRPRegistry.INTEGRITY_ALGORITHMS[i])
			.append(",");
	}
	if (confidentiality)
	{
		...
		cipher = CipherFactory
			.getInstance(SRPRegistry.CONFIDENTIALITY_ALGORITHMS[i]);
		if (cipher != null)
		    sb.append(SRPRegistry.OPTION_CONFIDENTIALITY).append("=")
			    .append(SRPRegistry.CONFIDENTIALITY_ALGORITHMS[i])
			    .append(",");
	    }
	}
	...
	srp = SRP.instance(
		mda == null ? SRPRegistry.SRP_DEFAULT_DIGEST_NAME : mda);
    }
	...
	while (st.hasMoreTokens())
	{
		...
		if (!integrity)
		    throw new AuthenticationException(
			    "Unknown integrity algorithm: " + option);
	    }
	    else if (option
		...
		if (!confidentiality)
		    throw new AuthenticationException(
			    "Unknown confidentiality algorithm: " + option);
	    }
	    else if (option
	...
	try
	{
	    expected = srp.generateM1(N, g, U, s, A, B, K, authorizationID, L,
		    cn, cCB);
	}
	...
	{
	    final SecurityContext ctx = ServerStore.instance()
		    .restoreSession(sid);

...
package com.distrimind.gnu.jgnu.security.key.rsa;

import java.math.BigInteger;
import java.security.AccessController;

import com.distrimind.gnu.jgnu.security.Registry;
import com.distrimind.gnu.jgnu.security.action.GetPropertyAction;
...
public class GnuRSAPrivateKey extends GnuRSAKey implements PrivateKey, RSAPrivateCrtKey {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6407366033638179521L;
	 ...
	 *
	 * @param p
	 *            the modulus first prime divisor.
	 * @param q
	 *            the modulus second prime divisor.
	 * @param e
	 *            the public exponent.
	 * @param d
	...
	public GnuRSAPrivateKey(int preferredFormat, BigInteger p, BigInteger q, BigInteger e, BigInteger d) {
		this(preferredFormat, p.multiply(q), e, d, p, q, e.modInverse(p.subtract(BigInteger.ONE)),
				e.modInverse(q.subtract(BigInteger.ONE)), q.modInverse(p));
	}
	 ...
	 *            externalizing this key.
	 * @param n
	 *            the public modulus, which is also the product of <code>p</code>
	 *            and <code>q</code>.
	 * @param e
	 *            the public exponent.
	 * @param d
	 ...
	 *            the modulus second prime divisor.
	 * @param dP
	 *            the first prime's exponen. A positive integer less than
	 *            <code>p</code> and <code>q</code>, satisfying
	 *            <code>e * dP = 1 (mod p-1)</code>.
	 * @param dQ
	 *            the second prime's exponent. A positive integer less than
	 *            <code>p</code> and <code>q</code>, satisfying
	 *            <code>e * dQ = 1 (mod p-1)</code>.
	 * @param qInv
	 *            the Chinese Remainder Theorem coefiicient. A positive integer less
	 *            than <code>p</code>, satisfying <code>q * qInv = 1 (mod p)</code>.
	 */
	public GnuRSAPrivateKey(int preferredFormat, BigInteger n, BigInteger e, BigInteger d, BigInteger p, BigInteger q,
			BigInteger dP, BigInteger dQ, BigInteger qInv) {
		super(preferredFormat == Registry.ASN1_ENCODING_ID ? Registry.PKCS8_ENCODING_ID : preferredFormat, n, e);
		this.d = d;
		this.p = p;
		this.q = q;
	...
	public boolean equals(final Object obj) {
		...
		if (obj instanceof RSAPrivateCrtKey) {
			...
			return super.equals(that) && p.equals(that.getPrimeP()) && q.equals(that.getPrimeQ())
					&& dP.equals(that.getPrimeExponentP()) && dQ.equals(that.getPrimeExponentQ())
	...
	public String toString() {
		if (str == null) {
			...
			str = new StringBuilder(this.getClass().getName()).append("(").append(super.toString()).append(",")
					.append(ls).append("d=0x").append("**...*").append(ls).append("p=0x").append("**...*").append(ls)
					.append("q=0x").append("**...*").append(ls).append("dP=0x").append("**...*").append(ls)
					.append("dQ=0x").append("**...*").append(ls).append("qInv=0x").append("**...*").append(ls)
					.append(")").toString();
		}
		return str;
	}

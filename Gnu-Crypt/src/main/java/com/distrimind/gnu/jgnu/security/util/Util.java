...
package com.distrimind.gnu.jgnu.security.util;
...
public class Util {
	 ...
	 *
	 * @param x
	 *            the {@link BigInteger} to display.
	 ...
	 * debugging. The string <code>m</code> is prepended to the start of each line.
	 * <p>
	 * If <code>offset</code> and <code>length</code> are omitted, the whole array
	 * is used. If <code>m</code> is omitted, nothing is prepended to each line.
	 *
	 * @param data
	 *            the byte array to be dumped.
	 * @param offset
	 *            the offset within <i>data</i> to start from.
	 * @param length
	 ...
	 * <p>
	 * Converts a string representing the encoding of some bytes in Base-64 to their
	 * original form.
	 *
	 * @param str
	 ...
	 *             if <code>str</code> is <code>null</code>, or <code>str</code>
	...
	/**
	 * Returns a number from <code>0</code> to <code>15</code> corresponding to the
	 * designated hexadecimal digit.
	 *
	 * @param c
	 ...
	 * @return unpadded (without the '=' character(s)) Base-64 representation of the
	 *         input.
	 */
	 ...
	 * <code>offset</code> and consisting of <code>length</code> bytes. Each byte is
	 ...
	 *            the byte array to convert.
	 * @param offset
	 *            the index from which to start considering the bytes to convert.
	 * @param length
	...
	/**
	 * Similar to the <code>toString()</code> method except that the Unicode escape
	 * character is inserted before every pair of bytes. Useful to externalise byte
	 * arrays that will be constructed later from such strings; eg. s-box values.
	 *
	 ...
	 * integer arrays that will be constructed later from such strings; eg. s-box
	 * values.
	 *
	 * @throws ArrayIndexOutOfBoundsException
	...
	/**
	 * Treats the input as the MSB representation of a number, and discards leading
	 * zero elements. For efficiency, the input is simply returned if no leading
	 * zeroes are found.
	 *
	 * @param n
	 *            the {@link BigInteger} to trim.

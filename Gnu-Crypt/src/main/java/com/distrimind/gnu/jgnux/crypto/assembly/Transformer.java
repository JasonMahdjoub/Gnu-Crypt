 ...
 * {@link Cascade} cipher, {@link com.distrimind.gnu.jgnux.crypto.pad.IPad} algorithm, and a
 ...
 * chain. Note that the same type <code>Transformer</code> can be used as either
 * in pre-processing or a post-processing modes.</li>
 * <li>A special transformer --<code>LoopbackTransformer</code>-- is used to
 * close the chain.</li>
 * <li>A useful type of <code>Transformer</code> --one we're interested in-- has
 * internal buffers. The distinction between a casual push (update) operation
 * and the last one allows to correctly flush any intermediate bytes that may
 * exist in those buffers.</li>
 * </ul>
 * <p>
 * To allow wiring <code>Transformer</code> instances together, a
 * <i>minimal-output-size</i> in bytes is necessary. The trivial case of a value
 * of <code>1</code> for such attribute practically means that no output
...
public abstract class Transformer {
	public static final String DIRECTION = "com.distrimind.gnu.crypto.assembly.transformer.direction";

...
package com.distrimind.gnu.jgnux.crypto.sasl;
...
public class SaslInputStream extends InputStream {
	 ...
	 * negotiated, the underlying <i>source</i> is expected to contain SASL buffers,
	 * as defined in RFC 2222. Four octets in network byte order in the front of
	 * each buffer identify the length of the buffer. The provider is responsible
	 * for performing any integrity checking or other processing on the buffer
	 * before returning the data as a stream of octets. For example, the protocol
	 * driver's request for a single octet from the stream might; i.e. an invocation
	 * of this method, may result in an entire SASL buffer being read and processed
	 ...
	 * <code>len</code> bytes, but a smaller number may be read, possibly zero. The
	 * number of bytes actually read is returned as an integer.
	 * <p>
	 * This method blocks until input data is available, end of file is detected, or
	 * an exception is thrown.
	 * <p>
	 * If <code>b</code> is <code>null</code>, a {@link NullPointerException} is
	 * thrown.
	 * <p>
	 ...
	 * equal to <code>len</code>. Let <code>k</code> be the number of bytes actually
	 * read; these bytes will be stored in elements <code>b[off]</code> through
	 * <code>b[off+k-1]</code>, leaving elements <code>b[off+k]</code> through
	 ...
	 * an {@link IOException} is thrown. In particular, an {@link IOException} is
	 * thrown if the input stream has been closed.
	 * <p>
	 * From the SASL mechanism provider's perspective, if a security layer has been
	 * negotiated, the underlying <i>source</i> is expected to contain SASL buffers,
	 * as defined in RFC 2222. Four octets in network byte order in the front of
	 * each buffer identify the length of the buffer. The provider is responsible
	 * for performing any integrity checking or other processing on the buffer
	 * before returning the data as a stream of octets. The protocol driver's
	 * request for a single octet from the stream might result in an entire SASL
	 * buffer being read and processed before that single octet can be returned.
	 *
	 * @param b
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		if (len <= internalBuf.length) {
			...
			off += result;
			len -= result;
			int remaining; // count of bytes remaining in buffer after an
			// iteration
	...
	private byte[] readSaslBuffer() throws IOException {
		...
		int bufferLength = result[0] << 24 | (result[1] & 0xFF) << 16 | (result[2] & 0xFF) << 8 | (result[3] & 0xFF);
		...
		if (realLength != bufferLength)
			throw new IOException("Was expecting " + bufferLength + " but found " + realLength);
		if (client != null)

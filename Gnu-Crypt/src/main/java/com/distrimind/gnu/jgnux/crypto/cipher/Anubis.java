 ...
 * <ol>
 * <li><a href=
 * "http://planeta.terra.com.br/informatica/paulobarreto/AnubisPage.html">The
...
public final class Anubis extends BaseCipher {
	...
	static {
		...
		for (i = 0, s = 0; i < 18;)
			rc[i++] = S[(s++) & 0xFF] << 24 | (S[(s++) & 0xFF] & 0xFF) << 16 | (S[(s++) & 0xFF] & 0xFF) << 8
					| (S[(s++) & 0xFF] & 0xFF);
	...
	private static void anubis(byte[] in, int i, byte[] out, int j, int[][] K) {
		...
		int a0 = (in[i++] << 24 | (in[i++] & 0xFF) << 16 | (in[i++] & 0xFF) << 8 | (in[i++] & 0xFF)) ^ Ker[0];
		int a1 = (in[i++] << 24 | (in[i++] & 0xFF) << 16 | (in[i++] & 0xFF) << 8 | (in[i++] & 0xFF)) ^ Ker[1];
		int a2 = (in[i++] << 24 | (in[i++] & 0xFF) << 16 | (in[i++] & 0xFF) << 8 | (in[i++] & 0xFF)) ^ Ker[2];
		int a3 = (in[i++] << 24 | (in[i++] & 0xFF) << 16 | (in[i++] & 0xFF) << 8 | (in[i] & 0xFF)) ^ Ker[3];
		int b0, b1, b2, b3;
		// round function
		for (int r = 1; r < R; r++) {
			Ker = K[r];
			b0 = T0[a0 >>> 24] ^ T1[a1 >>> 24] ^ T2[a2 >>> 24] ^ T3[a3 >>> 24] ^ Ker[0];
			b1 = T0[(a0 >>> 16) & 0xFF] ^ T1[(a1 >>> 16) & 0xFF] ^ T2[(a2 >>> 16) & 0xFF] ^ T3[(a3 >>> 16) & 0xFF]
					^ Ker[1];
			b2 = T0[(a0 >>> 8) & 0xFF] ^ T1[(a1 >>> 8) & 0xFF] ^ T2[(a2 >>> 8) & 0xFF] ^ T3[(a3 >>> 8) & 0xFF] ^ Ker[2];
			b3 = T0[a0 & 0xFF] ^ T1[a1 & 0xFF] ^ T2[a2 & 0xFF] ^ T3[a3 & 0xFF] ^ Ker[3];
			a0 = b0;
			a1 = b1;
			a2 = b2;
			a3 = b3;
	 ...
	 *
	 * @param uk
	 *            the 32N-bit user-supplied key material; 4 &lt;= N &lt;= 10.
	 * @param bs
	...
	public Object makeKey(byte[] uk, int bs) throws InvalidKeyException {
		...
		int[] tk = new int[N];
		int[] kk = new int[N];
		int r, i, k, k0, k1, k2, k3, tt;
		// apply mu to k0
		for (r = 0, i = 0; r < N;)
			tk[r++] = uk[i++] << 24 | (uk[i++] & 0xFF) << 16 | (uk[i++] & 0xFF) << 8 | (uk[i++] & 0xFF);
		for (r = 0; r <= R; r++) {
			if (r > 0) {
				// psi = key evolution function
				kk[0] = T0[(tk[0] >>> 24)] ^ T1[(tk[N - 1] >>> 16) & 0xFF] ^ T2[(tk[N - 2] >>> 8) & 0xFF]
						^ T3[tk[N - 3] & 0xFF];
				kk[1] = T0[(tk[1] >>> 24)] ^ T1[(tk[0] >>> 16) & 0xFF] ^ T2[(tk[N - 1] >>> 8) & 0xFF]
						^ T3[tk[N - 2] & 0xFF];
				kk[2] = T0[(tk[2] >>> 24)] ^ T1[(tk[1] >>> 16) & 0xFF] ^ T2[(tk[0] >>> 8) & 0xFF]
						^ T3[tk[N - 1] & 0xFF];
				kk[3] = T0[(tk[3] >>> 24)] ^ T1[(tk[2] >>> 16) & 0xFF] ^ T2[(tk[1] >>> 8) & 0xFF] ^ T3[tk[0] & 0xFF];
				for (i = 4; i < N; i++)
					kk[i] = T0[tk[i] >>> 24] ^ T1[(tk[i - 1] >>> 16) & 0xFF] ^ T2[(tk[i - 2] >>> 8) & 0xFF]
							^ T3[tk[i - 3] & 0xFF];
			...
			for (k = N - 2; k >= 0; k--) {
				tt = tk[k];
				k0 = T4[tt >>> 24] ^ (T5[(k0 >>> 24) & 0xFF] & 0xFF000000) ^ (T5[(k0 >>> 16) & 0xFF] & 0x00FF0000)
				...
				k3 = T4[tt & 0xFF] ^ (T5[(k3 >>> 24) & 0xFF] & 0xFF000000) ^ (T5[(k3 >>> 16) & 0xFF] & 0x00FF0000)
			...
			if (r == 0 || r == R) {
				...
				Kd[R - r][0] = T0[S[k0 >>> 24] & 0xFF] ^ T1[S[(k0 >>> 16) & 0xFF] & 0xFF]
						^ T2[S[(k0 >>> 8) & 0xFF] & 0xFF] ^ T3[S[k0 & 0xFF] & 0xFF];
				Kd[R - r][1] = T0[S[k1 >>> 24] & 0xFF] ^ T1[S[(k1 >>> 16) & 0xFF] & 0xFF]
						^ T2[S[(k1 >>> 8) & 0xFF] & 0xFF] ^ T3[S[k1 & 0xFF] & 0xFF];
				Kd[R - r][2] = T0[S[k2 >>> 24] & 0xFF] ^ T1[S[(k2 >>> 16) & 0xFF] & 0xFF]
						^ T2[S[(k2 >>> 8) & 0xFF] & 0xFF] ^ T3[S[k2 & 0xFF] & 0xFF];
				Kd[R - r][3] = T0[S[k3 >>> 24] & 0xFF] ^ T1[S[(k3 >>> 16) & 0xFF] & 0xFF]
						^ T2[S[(k3 >>> 8) & 0xFF] & 0xFF] ^ T3[S[k3 & 0xFF] & 0xFF];
			}
		}

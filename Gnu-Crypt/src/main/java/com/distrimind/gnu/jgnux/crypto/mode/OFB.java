...
package com.distrimind.gnu.jgnux.crypto.mode;
 ...
 * parallel. However, if the <code>IV</code> is known, the output blocks can be
 * generated prior to the availability of the plaintext or ciphertext data.
 * <p>
 * The OFB mode requires a unique <code>IV</code> for every message that is ever
 * encrypted under the given key. If, contrary to this requirement, the same
 * <code>IV</code> is used for the encryption of more than one message, then the
 * confidentiality of those messages may be compromised. In particular, if a
 * plaintext block of any of these messages is known, say, the j<sup>th</sup>

...
public class CipherFactory implements Registry {
	 ...
	 *            algorithm.
	 * @return an instance of the designated cipher algorithm, or <code>null</code>
	 *         if none is found.
	...
	public static final IBlockCipher getInstance(String name) {
		...
		if (name.equalsIgnoreCase(ANUBIS_CIPHER))
			...
			result = new Khazad();
		else if (name.equalsIgnoreCase(RIJNDAEL_CIPHER) || name.equalsIgnoreCase(AES_CIPHER))
			result = new Rijndael();
		else if (name.equalsIgnoreCase(SERPENT_CIPHER))
			result = new Serpent();
		else if (name.equalsIgnoreCase(SQUARE_CIPHER))
			result = new Square();
		else if (name.equalsIgnoreCase(TRIPLEDES_CIPHER) || name.equalsIgnoreCase(DESEDE_CIPHER))

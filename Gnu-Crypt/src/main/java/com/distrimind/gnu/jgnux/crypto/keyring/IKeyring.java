...
/**
 * The top-level interface to a <i>keyring:</i> a file that is used to store and
 * protect public and private cryptographic keys.
 * <p>
 * A <i>keyring</i> is modelled as a mapping of one <i>alias</i> to one or more
 * <i>entries</i> (optionally of different types).
 * <p>
...
public interface IKeyring {
	 ...
	 */
	public static final String KEYRING_DATA_IN = "com.distrimind.gnu.crypto.keyring.data.in";
	 ...
	 */
	public static final String KEYRING_PASSWORD = "com.distrimind.gnu.crypto.keyring.password";
	...
	/**
	 * Returns a {@link List} of entries (instances of {@link Entry}) for the given
	 * <code>alias</code>, or <code>null</code> if there no such entry exists.
	 *
	 * @param alias
	 *            The alias of the entry(ies) to return.
	 * @return A list of all entries (instances of {@link Entry} that have the given
	 *         <code>alias</code>, or <code>null</code> if no one {@link Entry} can
	 *         be found with the designated <code>alias</code>.
	 */
	List<Entry> get(String alias);

	/**
	 * Loads a keyring into memory.
	 * <p>
	 * What happens to the current contents of this keyring? are the new ones merged
	 * with the current ones or do they simply replace them?
	 *
	 * @param attributes
	 *            The attributes that designate the source where the store is to be
	 *            loaded from. What happens
	 * @throws IllegalArgumentException
	 *             If the attributes are inappropriate.
	 * @throws IOException
	 *             If the keyring file cannot be read.
	 * @throws SecurityException
	 *             If the given password is incorrect, or if the top-level
	 *             authentication or decryption fails.
	 */
	void load(Map<String, Object> attributes) throws IOException;

	/**
	 * Removes an entry with the designated <code>alias</code> from this keyring.
	 * Does nothing if there was no such entry.
	 * <p>
	...
	/**
	 * Stores the contents of this keyring to persistent storage as specified by the
	 * designated <code>attributes</code>.
	 *
	 * @param attributes
	 *            the attributes that define where the contents of this keyring will
	 *            be stored.
	 * @throws IOException
	 *             if an exception occurs during the process.
	 */
	void store(Map<String, Object> attributes) throws IOException;
}
